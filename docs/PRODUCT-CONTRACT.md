...
- When choices help, use consecutive stable labels from `A` through at most `G`; two or three choices remain the default, and every additional route must be genuinely distinct.
...
- Repeated agreement alone never creates delegation. Increment the durable recommended-key streak only when the user's complete trimmed reply is exactly the recommended option letter, case-insensitively. Added words or punctuation, another key, a non-recommended choice, a digression, redirect, or protected gate resets it.
- After three qualifying replies, if another reversible human-owned question exists, keep that real question's recommended answer as `A`, insert `B` as “use my recommendations for every remaining reversible decision,” and relabel its other routes consecutively from `C` through at most `G`. If insertion would exceed seven choices, combine overlaps or omit the least consequential ordinary route while retaining the custom-answer path; never emit `H`. Do not ask a separate delegation question. If the next step is already a protected gate, reset interaction state to `0 / not ready` and proceed to it. Only a bare `B` answer grants authority.
- Bare `B` to that inserted shortcut applies the recommended answer to the current question, synthesizes all remaining reversible choices in the same turn, and stops at the first protected gate. Any other response consumes the offer and resolves the current question normally; a later new three-key streak may expose it again.
- After a side question, challenge, or context-rich paragraph, reset the streak, answer or reconcile it, recompute the current decision, and end with the complete refreshed `A` through at most `G` choice set whenever one worthwhile human decision remains. Do not repeat a stale choice or end with an open prompt merely because the reply required explanation.
