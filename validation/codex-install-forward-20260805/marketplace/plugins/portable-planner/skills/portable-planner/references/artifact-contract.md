...
`PLAN.md`, planning tickets, evidence, and execution tickets are canonical. `PLAN-VIEW.md` is a generated view of that state and must never contain a decision that is absent from canonical files.
...
Create `evidence/P-NNN-evidence.md` only when external facts affect a decision. Prefer primary/direct sources. Record source, access date, relevant finding, and which choice it changes. Stop when additional sources are unlikely to change the decision.
