...
A successor task changes the user's task list. Create one only when the user has explicitly authorized automatic task continuation for this plan. Record that plan-scoped permission in `PLAN.md` as `Continuation: automatic — authorized`. A natural planning request by itself does not grant task-creation permission. If permission is absent when a real boundary appears, ask once whether to create the next task or leave a paste-ready handoff.

Permission covers planning successors only. Finished-plan approval does not authorize implementation or a build task; use the host's normal build workflow only after the user explicitly asks to proceed.
...
If the host has no task-creation capability, or creation fails, preserve state and show this one paste-ready recovery action:

```text
Use $portable-planner. Follow /absolute/path/to/planning/NEXT.md.
```

Never claim a successor exists until the host returns success. Never hide an uncertain result by retrying and risking duplicates.
