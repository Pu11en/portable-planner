...
- Outcome: Drew experiences the objectively passing candidate in one fresh uncoached task, with automatic restoration to public beta 6 after a failure.
...
- For a kept candidate, record the current public beta-6 installation, install the exact candidate bytes in controlled user scope, verify its version/source, and open one dedicated empty test project.
...
- On any failure or “worse” judgment, immediately restore and verify public beta 6; mark the candidate rejected.
- If E-012 justified no candidate, run the unchanged beta-6 human test instead and preserve that result.
...
- Failure proof includes successful beta-6 restoration; pass proof identifies the unchanged candidate eligible for release.

## If blocked or disproven

- Restore beta 6 and record the blocker or first failure. Reopen only the shared behavior that caused it.
