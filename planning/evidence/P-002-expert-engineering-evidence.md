...
## Cross-domain repository pass

Primary sources inspected on 2026-08-15 after Drew clarified that outside work is inspiration for Portable Planner, never a replacement:

- Every's [Compound Engineering `ce-plan`](https://github.com/EveryInc/compound-engineering-plugin/blob/main/docs/skills/ce-plan.md) explicitly uses one planning engine for software, maintenance, study plans, trips, research workflows, events, and personal projects. Its useful abstraction is **guardrails over choreography**: preserve decisions, scope, stable work-unit identity, dependencies, risks, and test scenarios while leaving domain-specific execution mechanics to the implementer. Its [execution companion](https://github.com/EveryInc/compound-engineering-plugin/blob/main/docs/skills/ce-work.md) triages trivial clear work directly, uses a lightweight task list for small/medium work, and recommends planning for large or sensitive work. This is strong evidence for an adaptive trigger instead of forcing the entire planning ceremony onto every message.
- Matt Pocock's [Wayfinder](https://github.com/mattpocock/skills/blob/main/skills/engineering/wayfinder/SKILL.md) keeps a destination-fixed frontier, distinguishes human conversation from agent research, and leaves the future in “fog” until its question can be stated precisely. Portable Planner already embodies much of this; the value is an independent check on not pre-slicing unknown work or letting an agent answer the human side of a decision.
- Forward Future's [Loopy](https://github.com/Forward-Future/loopy) defines reusable improvement loops through four domain-independent questions: goal, evidence of success, response to what was learned, and stopping/handoff condition. The transferable part is a small bounded feedback contract, not its catalog or publication system.
- Addy Osmani's [Agent Skills](https://github.com/addyosmani/agent-skills) separates routing, one-question interviewing, idea refinement, specification, planning, implementation, and verification. Its lifecycle is software-centered, so it can inform route separation but cannot prove Portable Planner's cross-domain experience.
- Sage and GStack provide stronger engineering review gates but remain predominantly product/software workflows. They are useful negative and positive controls for review independence, routing, and regression discipline—not candidates to become Portable Planner's protocol.

No inspected repository provides direct evidence that one complete conversational planning protocol is already effective across all of Portable Planner's target domains. The defensible route is selective adaptation: every mechanism must cite the Portable Planner failure it addresses, avoid a second framework or state tree, and pass at least one software and materially different non-software case before it can be called general.
